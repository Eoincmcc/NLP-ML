...
class DenseIndexer(object):
    ...
    def __init__(self, buffer_size: int = 50000):
        ...
        self.index_id_to_db_id = [] # 1.initialised here
        self.index = None
...
class DenseFlatIndexer(DenseIndexer):
    ...
    def index_data(self, data: List[Tuple[object, np.array]]):
        ...
        indexed_cnt = len(self.index_id_to_db_id) # 2. Has length of 0 here?
        logger.info('Total data indexed %d', indexed_cnt)

    def search_knn(self, query_vectors: np.array, top_docs: int) -> List[Tuple[List[object], List[float]]]:
        scores, indexes = self.index.search(query_vectors, top_docs)
        # convert to external ids
        db_ids = [[self.index_id_to_db_id[i] for i in query_top_idxs] for query_top_idxs in indexes] # 3. Empty here also
